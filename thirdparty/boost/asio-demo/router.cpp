...
void Router::register_dynamic_handler(http::verb method,
                                      const std::string &path_template,
                                      ApiHandlerPtr handler)
{
    ...
    while (std::regex_search(it, path_template.cend(), matches,
                             std::regex("\\{([^}]+)\\}")))
    {
    ...
    get_dynamic_routes().emplace_back(
        method,
        std::make_tuple(
            std::regex(regex_str),
            param_names,
...
std::pair<ApiHandler *, std::unordered_map<std::string, std::string>> Router::route(http::verb method, const std::string &path)
{
    if (auto it = get_handlers().find({method, path});
        it != get_handlers().end())
    {
